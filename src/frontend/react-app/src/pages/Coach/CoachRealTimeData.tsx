...
export default function CoachStartSessionPage() {
  ...
  const [sessionRealTimeData, setSessionInfo] = useState<SessionRealTimeData>();
  ...
  useEffect(() => {
    const checkIsHistorical = async () => {
      try {
        ...
        console.error("Error checking if session is historical:", error);
        return false;
      }
    };
              ...
              >
                ...
                <div className="w-full flex justify-start gap-4">
                  ...
                  <PlayerUniqueCard
                    ...
                    playerName={
                      !isHistorical ? "Heart Rate" : "Average Heart Rate"
                    }
                    playerId={player.playerId.toString()}
                    singleValue={
                      !isHistorical
                        ? player.heartRateData.at(-1)?.value?.toString() ||
                          "N/A"
                    ...
                    singleValue={
                      !isHistorical
                        ? player.bodyTemperatureData.at(-1)?.value.toString() ||
                          "N/A"
                        : player.bodyTemperatureData.length > 0
                        ? (
                            player.bodyTemperatureData.reduce(
                              (acc, curr) => acc + curr.value,
                              0
                            ) / player.bodyTemperatureData.length
                          ).toFixed(1)
                        : "N/A"
                    }
                    actualState={
                    ...
                    singleValue={
                      !isHistorical
                        ? player.respiratoryRateData.at(-1)?.value.toString() ||
                          "N/A"
                        : player.respiratoryRateData.length > 0
                        ? (
                            player.respiratoryRateData.reduce(
                              (acc, curr) => acc + curr.value,
                              0
                            ) / player.respiratoryRateData.length
                          ).toFixed(0)
                        : "N/A"
                    }
                    actualState={
